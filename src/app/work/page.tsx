    ...
    <div className="min-h-screen bg-[#FAFAF8] text-[#171717]">
      ...
      <main>
        <section className="mx-auto max-w-content px-5 py-20 md:px-8 md:py-24 lg:py-32">
          <p className="text-[12px] font-normal uppercase tracking-[0.18em] text-black/45">
            Selected Work
          </p>
          <h1 className="mt-4 text-[clamp(2rem,5.4vw,4rem)] font-normal leading-[1.08] text-[#171717]">
            Work Archive
          </h1>
          <p className="mt-6 max-w-2xl break-keep text-base font-normal leading-8 text-black/50 md:text-lg">
              ...
              <section key={group.category} aria-labelledby={`work-${group.category}`}>
                <div className="grid gap-5 border-t border-black/15 pt-6 md:grid-cols-[0.32fr_1fr] md:gap-10">
                  <h2
                    ...
                    className="text-[15px] font-normal leading-7 text-black/55 md:text-[18px]"
                  >
                    {group.category}
                  </h2>
                  <ul className="border-t border-black/10 md:border-t-0">
                    {group.projects.map((project) => (
                      <li
                        key={`${group.category}-${project.name}-${project.year}`}
                        className="grid gap-1 border-b border-black/10 py-4 text-sm font-normal leading-6 text-black/55 transition hover:bg-black/[0.025] md:grid-cols-[1fr_1fr_auto] md:gap-6 md:px-2 md:text-[15px]"
                      >
                        <span className="break-keep text-black/80">{project.name}</span>
